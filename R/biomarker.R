...
summary.biomarker  <-  function(x, ...){

  cat("Number of identified biomarkers: ",
      sum(x$biom.ident$biomarkers),"\n")


  if (is.null(x$biom.ident$names)){
    x$biom.ident$names  <- 1:length(x$biom.ident$V)
  }
  names(x$biom.ident$Vstar) <- x$biom.ident$names
  cat("Sorted values of V* statistic: ","\n","\n")
  cat("Biomarkers:","\n")
  print(sort(x$biom.ident$Vstar[x$biom.ident$Vstar>x$cut],
             decreasing=TRUE))
  cat("\n")
  cat("Non-Biomarkers:","\n")
  print(sort(x$biom.ident$Vstar[x$biom.ident$Vstar<=x$cut],decreasing=TRUE))
  cat("\n")
  cat("Used cut-off value: ",x$cut)
}
